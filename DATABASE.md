...
#### 4. Bảng `businessEvents` (Nhật ký Sự kiện Nhịp vụ - system.businessEvents)

Bảng này dùng để lưu lại các sự kiện (timeline) quan trọng của nhiều bảng/đối tượng khác nhau trong hệ thống theo thời gian thực (nhằm phục vụ cho việc kiểm vết, đánh giá tiến độ hoặc hiển thị Timeline component).

Các loại sự kiện chính được quy định:

- **incomingRequests**: đã thanh toán/ thanh toán 1 phần (`Paid`, `Partial`).
- **receipts**: tạo mới (`Draft`/`Received`), hoàn thành (`Completed`, `Reported`), hủy bỏ (`Cancelled`).
- **samples**: tạo mới (`Received`), lưu mẫu (`Retained`), trả khách hàng (`Returned`), hủy mẫu (`Disposed`).
- **analyses**: tạo mới (`Pending`), nhận bàn giao (`HandedOver`), nhập liệu sơ bộ (`DataEntered`), duyệt kết quả (`Approved`), khiếu nại (`Complained`), làm lại (`ReTest`), hủy bỏ (`Cancelled`).

| Column Name       | Type        | Key    | Description                                                                 |
| :---------------- | :---------- | :----- | :-------------------------------------------------------------------------- |
| `eventId`         | `text`      | **PK** | Mã sự kiện (VD: `EVT_1020304`).                                             |
| `entityType`      | `text`      |        | Phân loại đối tượng (`incomingRequest`, `receipt`, `sample`, `analysis`).   |
| `entityId`        | `text`      | **FK** | ID của đối tượng phát sinh sự kiện (Ref type).                              |
| `eventStatus`     | `text`      |        | Trạng thái tương ứng (VD: `ReTest`, `Paid`, `Completed`, `HandedOver`...).  |
| `eventNote`       | `text`      |        | Ghi chú bổ sung (nếu có, VD: "Thanh toán lần 1: 5.000.000").                |
| `eventData`       | `jsonb`     |        | Dữ liệu chi tiết snapshot tại thời điểm xảy ra sự kiện.                     |
| `involvedIds`     | `text[]`    | **FK** | Những người liên quan (`identity.identities`) nhận thông báo của event.     |
| `createdAt`       | `timestamp` |        | Thời gian xảy ra sự kiện.                                                   |
| `createdById`     | `text`      | **FK** | User thực hiện sự kiện.                                                     |
