...
| `paymentStatus` | `text` | | `Unpaid`, `Partial`, `Paid`, `Variance`, `Debt`. |
...
| `displayStyle` | `jsonb` | | Cấu hình hiển thị báo cáo: `{"eng": "...", "vie": "..."}`. |
...
| `displayStyle` | `jsonb` | | Cấu hình hiển thị dạng markdown `{"eng": "<tên tiếng anh>", "vie": "<tên tiếng Việt>"}`. |
...
| `analysisReportDisplay` | `jsonb` | | **(Object)** Định dạng hiển thị trên báo cáo `{"eng": <text>, "vie": <text>}`. |
