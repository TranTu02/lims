export default {
    nav: {
        receptionTitle: "Sample Reception",
        receptionDescription: "Manage receipts & Samples",
        incomingRequestsTitle: "Incoming Requests",
        incomingRequestsDescription: "Client sample requests",
        returnResultsTitle: "Return Results",
        returnResultsDescription: "Analysis results & shipments",

        technicianTitle: "KTV Workspace",
        technicianDescription: "Testing work",

        equipmentTitle: "Equipment",
        equipmentDescription: "Lab equipment & devices",

        managerTitle: "Lab Management",
        managerDescription: "Review results & Reports",
        managerApprovals: "Review & Approve",
        managerApprovalsDesc: "Review and approve results",
        managerExceptions: "Exceptions",
        managerExceptionsDesc: "Urgent, complaints, retest",
        managerSamples: "Processing Samples",
        managerSamplesDesc: "Sample progress & handover",
        managerAnalyses: "Processing Analyses",
        managerAnalysesDesc: "Assignment & chemicals",
        managerReports: "Reports",
        managerReportsDesc: "Statistics & reporting",

        assignmentTitle: "Technician Assignment",
        assignmentDescription: "Assign Parameters",

        handOverTitle: "Handover",
        handOverDescription: "Inter-department Handover",

        storedSamplesTitle: "Stored Samples",
        storedSamplesDescription: "Sample Storage",


        libraryTitle: "Library",
        libraryDescription: "System Library",

        parameterTitle: "Parameters",
        parameterDescription: "Parameter Library",

        protocolsTitle: "Methods",
        protocolsDescription: "Method Library",

        matricesTitle: "Parameter Matrices",
        matricesDescription: "Parameter Matrix Library",

        sampleTypesTitle: "Sample Types",
        sampleTypesDescription: "Sample Type Library",

        parameterGroupsTitle: "Parameter Groups",
        parameterGroupsDescription: "Parameter Group Library",

        documentTitle: "Documents",
        documentDescription: "Forms & Templates",

        inventoryTitle: "Inventory & Assets",
        inventoryDescription: "Equipment & Assets",

        chemicalInventoryTitle: "Chemical Inventory",
        chemicalInventoryDescription: "Chemicals & Reagents",

        generalInventoryTitle: "General Inventory",
        generalInventoryDescription: "Manage equipment and lab tools",
        stationeryTitle: "Stationery",
        stationeryDescription: "Manage stationery and office supplies",

        crmTitle: "Customers",
        crmDescription: "Customer relationship management",

        hrTitle: "Human Resources",
        hrDescription: "Employee Management",
        sidebar: {
            title: "LIMS Lab",
            description: "Lab Management",
            collapse: "Collapse",
            expand: "Expand",
            language: "Language",
            langVi: "Vietnamese",
            langEn: "English",
            theme: "Theme",
            themeLight: "Light",
            themeDark: "Dark",
            themeSystem: "System",
            dashboard: "Dashboard",
            orders: "Orders",
            samples: "Samples",
            clients: "Clients",
            inventory: "Assets & Inventory",
            chemicalInventory: "Chemical Inventory",
            accounting: "Accounting",
            settings: "Settings",
        },
    },

    notifications: {
        title: "Notifications",
        markAllRead: "Mark all as read",
        markRead: "Mark as read",
        delete: "Delete notification",
        empty: "No new notifications",
        emptyUnread: "No unread notifications",
        unreadCount: "{{count}} unread notifications",
        all: "All",
        unread: "Unread",
        tabs: {
            all: "All",
            unread: "Unread",
        },
    },
    common: {
        receipt: "Receipt",
        view: "View",
        back: "Back",
        edit: "Edit",
        save: "Save",
        saving: "Saving",
        select: "Select",
        cancel: "Cancel",
        delete: "Delete",
        add: "Add",
        all: "All",
        create: "Create",
        search: "Search...",
        filter: "Filter",
        export: "Export",
        import: "Import",
        actions: "Actions",
        noData: "No data",
        loading: "Loading...",
        success: "Success",
        error: "Error",
        upload: "Upload",
        fileName: "File Name",
        type: "Type",
        size: "Size",
        uploadedBy: "Uploaded By",
        uploadedAt: "Uploaded At",
        field: "Field",
        value: "Value",
        empty: "No data",
        count: "Total",
        apply: "Apply",
        clear: "Clear",
        toast: {
            saved: "Saved successfully",
            created: "Created successfully",
            deleted: "Deleted successfully",
            failed: "Action failed",
            success: "Action successfully",
            tryAgain: "Try Again",
            requestFailed: "Request Failed",
        },
        filterBy: "Filter by:",
        clearFilter: "Clear filter",
        from: "From",
        to: "To",
        extractionDate: "Extraction date",
        refresh: "Refresh",
        placeholder: {
            enterValue: "Enter value...",
            enterField: "Enter field name...",
            clickToAdd: 'Click "Add" to add info',
        },
        close: "Close",
        quantity: "Quantity",
        createdAt: "Created At",
        createdById: "Created By",
        modifiedAt: "Last Modified",
        modifiedById: "Modified By",
        deletedAt: "Deleted At",
        status: "Status",
        note: "Note",
        none: "None",
        prev: "Previous",
        next: "Next",
        page: "Page",
        errorTitle: "An error occurred",
        confirmDelete: "Are you sure you want to delete? This action cannot be undone.",
        pagination: {
            itemsPerPagePrefix: "Show",
            itemsPerPageSuffix: " / page",
            itemsPerPageAria: "Rows per page",
            range: "{{start}}-{{end}} of {{total}}",
            firstPage: "First page",
            prevPage: "Previous page",
            nextPage: "Next page",
            lastPage: "Last page",
            page: "Page {{page}}",
        },
        reportPreparedBy: "Report Prepared By",
        signAndWriteName: "(Sign and write full name)",
    },
    theme: {
        light: "Light",
        dark: "Dark",
        system: "System",
        toggle: "Toggle Theme",
    },
    identity: {
        identities: {
            identityId: "Identity ID",
            email: "Email",
            identityName: "Full Name",
            alias: "Alias",
            roles: "System Roles",
            permissions: "Detailed Permissions",
            password: "Password",
            identityStatus: "Account Status",
        },
        sessions: {
            sessionId: "Session ID",
            identityId: "Identity ID",
            sessionExpiry: "Expiry Time",
            sessionStatus: "Session Status",
            ipAddress: "IP Address",
            sessionDomain: "Access Domain",
        },
    },
    shipments: {
        title: "Shipments",
        subtitle: "Manage sample and result delivery",
        senderName: "INSTITUTE OF RESEARCH AND DEVELOPMENT OF NATURAL PRODUCTS",
        senderAddress: "176 Phung Khoang, Dai Mo Ward, Hanoi, Vietnam",
        senderPhone: "0868872578",
        senderEmail: "quantri@irdop.org",
    },
    crm: {
        title: "Customers",
        subtitle: "Customer relationship management",

        tabs: {
            clients: "Customers",
            orders: "Orders",
            quotes: "Quotes",
        },
        clients: {
            clientId: "Client ID",
            clientName: "Client Name / Company",
            legalId: "Tax ID / ID Card",
            clientAddress: "Address",
            clientPhone: "Phone",
            clientEmail: "Contact Email",
            clientSaleScope: "Management Scope",
            availableByIds: "Sales Rep (IDs)",
            availableByName: "Sales Rep (Name)",
            contacts: "Contact Person",
            invoiceInfo: "Invoice Info",
            totalOrderAmount: "Total Accumulated Sales",
            create: {
                title: "Create customer",
            },
            update: {
                title: "Update customer",
            },

            placeholders: {
                clientId: "Enter client ID...",
                clientName: "Enter customer/company name...",
                legalId: "Enter tax/ID...",
                saleScope: "Select management scope...",
                phone: "Enter phone number...",
                email: "Enter email...",
                address: "Enter address...",
            },
            sections: {
                base: {
                    title: "Basic information",
                },

                available: {
                    title: "Sales assignment",
                    add: "Add",
                    empty: "No sales assignment",
                    itemTitle: "Sales #{{index}}",
                    byIds: "Sales Rep (IDs)",
                    byName: "Sales Rep (Name)",
                    byIdsPlaceholder: "e.g.",
                    byNamePlaceholder: "e.g.",
                },

                invoice: {
                    title: "Invoice information",
                    taxName: "Company name",
                    taxCode: "Tax code",
                    taxEmail: "Email",
                    taxAddress: "Address",
                    taxNamePlaceholder: "Enter company name...",
                    taxCodePlaceholder: "Enter tax code...",
                    taxEmailPlaceholder: "Enter email...",
                    taxAddressPlaceholder: "Enter address...",
                },

                contacts: {
                    title: "Contacts",
                    add: "Add",
                    empty: "No contacts",
                    itemTitle: "Contact #{{index}}",
                    fields: {
                        contactId: "Contact ID",
                        contactName: "Full name",
                        contactPhone: "Phone",
                        contactEmail: "Email",
                        contactPosition: "Position",
                        contactAddress: "Address",

                        contactIdPlaceholder: "Enter contact ID...",
                        contactNamePlaceholder: "Enter full name...",
                        contactPhonePlaceholder: "Enter phone...",
                        contactEmailPlaceholder: "Enter email...",
                        contactPositionPlaceholder: "Enter position...",
                        contactAddressPlaceholder: "Enter address...",
                    },
                },
            },
            saleScope: {
                public: "Public",
                private: "Private",
            },

            columns: {
                clientId: "Client ID",
                clientName: "Client Name",
                legalId: "Tax/ID",
                invoiceName: "Invoice Name",
                primaryContact: "Primary Contact",
                saleScope: "Scope",
                phone: "Phone",
                email: "Email",
                address: "Address",
                totalOrderAmount: "Total Revenue",
                availableByIds: "Sales Rep (IDs)",
                availableByName: "Sales Rep (Name)",
            },

            tooltip: {
                invoiceTitle: "Invoice Information",
                taxName: "Company Name",
                taxCode: "Tax Code",
                taxEmail: "Email",
                taxAddress: "Address",

                contactsTitle: "Contacts",
            },

            detail: {
                title: "Customer Details",
                noContacts: "No contacts",
                sections: {
                    basic: "Basic Information",
                    availability: "Sales Assignment",
                    invoiceInfo: "Invoice Info",
                    contacts: "Contacts",
                },
            },

            invoice: {
                taxCode: "Tax Code",
                taxName: "Company Name",
                taxEmail: "Email",
                taxAddress: "Address",
            },

            contact: {
                title: "Contact",
                phone: "Phone",
                email: "Email",
                position: "Position",
                address: "Address",
            },

            delete: {
                title: "Delete customer",
                description: "Are you sure you want to delete customer",
            },
        },
        orders: {
            title: "Order Detail",
            subtitle: "Manage orders, samples, and payments",

            tabs: {
                orders: "Orders",
            },

            columns: {
                orderId: "Order ID",
                quoteId: "Quote ID",
                clientId: "Client ID",
                salePersonId: "Sales Rep (ID)",
                saleCommissionPercent: "Commission %",
                totalAmount: "Total Amount",

                orderStatus: "Order Status",
                paymentStatus: "Payment Status",

                totalFeeBeforeTax: "Total Fee (Pre-tax)",
                totalFeeBeforeTaxAndDiscount: "Total Listed Fee",
                totalTaxValue: "Tax Value",
                totalDiscountValue: "Discount Value",
                taxRate: "Tax Rate (%)",
                discountRate: "Discount (%)",

                orderUri: "Sample Submission Link",
                entityType: "Entity Type",
            },

            status: {
                pending: "Pending",
                processing: "Processing",
                completed: "Confirmed",
                cancelled: "Cancelled",
            },

            payment: {
                unpaid: "Unpaid",
                partially: "Partially Paid",
                paid: "Paid",
                debt: "Debt",
            },
            paymentStatus: {
                Unpaid: "Unpaid",
                Partial: "Partially Paid",
                Partially: "Partially Paid",
                Paid: "Paid",
                Debt: "Debt",
            },

            orderStatus: {
                Pending: "Pending",
                Processing: "Processing",
                Completed: "Confirmed",
                Cancelled: "Cancelled",
            },

            create: { title: "Create Order" },
            update: { title: "Update Order" },

            form: {
                sections: {
                    basic: "Basic",
                    contact: "Contact",
                    requestForm: "Request Form",
                    samples: "Samples",
                    transactions: "Transactions",
                },
                placeholders: {
                    orderId: "Enter order ID...",
                    quoteId: "Enter quote ID...",
                    clientId: "Enter client ID...",
                    salePersonId: "Enter sales rep ID...",
                    saleCommissionPercent: "Enter commission %...",

                    totalAmount: "Enter total amount...",
                    taxRate: "Enter tax rate...",
                    discountRate: "Enter discount rate...",

                    orderStatus: "Select order status...",
                    paymentStatus: "Select payment status...",

                    contactName: "Enter contact name...",
                    contactEmail: "Enter email...",
                    contactPhone: "Enter phone...",

                    requestedBy: "Enter requested by...",
                    requestedAt: "Select date...",
                    requestNotes: "Enter notes...",

                    sampleName: "Enter sample name...",
                    userSampleId: "Enter client sample ID...",
                    sampleTypeId: "Enter sample type ID...",
                    sampleTypeName: "Enter sample type name...",
                    matrixId: "Enter matrix ID...",

                    txDate: "Select date...",
                    txMethod: "Enter method...",
                    txAmount: "Enter amount...",
                    txNote: "Enter note...",
                },
            },

            contact: {
                contactName: "Contact Name",
                contactEmail: "Email",
                contactPhone: "Phone",
            },

            requestForm: {
                requestedBy: "Requested By",
                requestedAt: "Requested At",
                notes: "Notes",
            },

            samples: {
                sample: "Sample",
                analyses: "Analyses",

                sampleId: "Sample ID",
                sampleTypeId: "Sample Type (ID)",
                sampleTypeName: "Sample Type",
                userSampleId: "Client Sample ID",
                matrixId: "Matrix ID",

                fields: {
                    sampleName: "Sample Name",
                    userSampleId: "Client Sample ID",
                    sampleTypeId: "Sample Type (ID)",
                    sampleTypeName: "Sample Type",
                    matrixId: "Matrix ID",
                },
            },

            transactions: {
                title: "Transaction",
                method: "Method",
                amount: "Amount",
                fields: {
                    date: "Date",
                    method: "Method",
                    amount: "Amount",
                    note: "Note",
                },
            },

            detail: {
                title: "Order Detail",
                sections: {
                    basic: "Basic",
                    amounts: "Amounts",
                    audit: "Audit",
                    client: "Client",
                    contactPerson: "Contact Person",
                    samples: "Samples & Analyses",
                    transactions: "Transactions",
                    requestForm: "Request Form",
                    reportRecipient: "Report Recipient",
                },
            },

            delete: {
                title: "Delete Order",
                description: "Are you sure you want to delete order",
            },
        },

        quotes: {
            columns: {
                quoteId: "Quote ID",
                quoteCode: "Quote Code",
                clientId: "Client ID",
                salePersonId: "Sales Rep (ID)",

                totalAmount: "Total Amount",
                quoteStatus: "Quote Status",

                taxRate: "Tax Rate (%)",
                discount: "Discount (%)",
                totalFeeBeforeTax: "Subtotal (Pre-tax)",
                totalFeeBeforeTaxAndDiscount: "Original Subtotal",
                totalTaxValue: "Tax Value",
                totalDiscountValue: "Discount Value",

                contactName: "Contact Name",
                contactEmail: "Email",
                contactPhone: "Phone",
                contactPosition: "Position",
            },

            placeholders: {
                quoteId: "Auto-generated...",
                quoteCode: "Enter quote code...",
                clientId: "Enter client ID...",
                salePersonId: "Enter sales rep ID...",

                totalAmount: "Enter total amount...",
                quoteStatus: "Select status...",

                taxRate: "Enter tax rate...",
                discount: "Enter discount...",

                totalFeeBeforeTax: "Enter subtotal (pre-tax)...",
                totalFeeBeforeTaxAndDiscount: "Enter original subtotal...",
                totalTaxValue: "Enter tax value...",
                totalDiscountValue: "Enter discount value...",

                contactName: "Enter contact name...",
                contactEmail: "Enter contact email...",
                contactPhone: "Enter phone...",
                contactPosition: "Enter position...",
            },

            status: {
                draft: "Draft",
                sent: "Sent",
                approved: "Approved",
                expired: "Expired",
            },

            create: { title: "Create Quote" },
            update: { title: "Update Quote" },

            form: {
                sections: {
                    basic: "Basic",
                    contact: "Contact",
                    amounts: "Amounts",
                },
            },

            detail: {
                title: "Quote Details",
                sections: {
                    basic: "Basic",
                    contact: "Contact",
                    amounts: "Amounts",
                    audit: "Audit",
                },
            },

            delete: {
                title: "Delete quote",
                description: "Are you sure you want to delete quote",
            },
        },
    },
    library: {
        matrices: {
            title: "Parameter Matrix List",
            total: "Total: {{count}} matrices",
            matrixId: "Parameter Matrix (ID)",
            parameterId: "Parameter ID",
            protocolId: "Protocol ID",
            sampleTypeId: "Sample Type",
            protocolCode: "Protocol Code",
            protocolSource: "Protocol Source",
            protocolAccreditation: "Accreditation Scope (VILAS/TDC)",
            parameterName: "Parameter Name",
            sampleTypeName: "Sample Type Name",
            feeBeforeTax: "Unit Price (Pre-tax)",
            taxRate: "Tax Rate (%)",
            feeAfterTax: "Unit Price (Post-tax)",
            methodLOD: "Limit of Detection (LOD)",
            methodLOQ: "Limit of Quantitation (LOQ)",
            thresholdLimit: "Standard Threshold",
            turnaroundTime: "Turnaround Time (Days)",
            technicianGroupId: "Responsible Tech Group",
            searchPlaceholder: "Search Matrices...",
            create: {
                title: "Create New Parameter Matrix",
                sampleParameter: "Parameter/Sample",
                protocol: "Protocol",
                pricing: "Pricing",
                limits: "Limits",
            },
            detail: {
                title: "Parameter Matrix Detail",
                sampleParameter: "Parameter/Sample",
                protocol: "Protocol",
                pricing: "Pricing",
                limits: "Limits",
                information: "Information",
            },
            edit: {
                title: "Edit Parameter Matrix",
            },
            cofirmDelete: "Are you sure you want to delete this matrix?",
        },
        protocols: {
            protocolId: "Protocol ID",
            protocolCode: "Standard Code (SOP)",
            protocolSource: "Issuing Organization",
            protocolAccreditation: {
                title: "Competence Certification",
                vilas: "VILAS",
                tdc: "TDC",
            },
            protocolCreateAt: "Created At",
            title: "Protocol List",
            total: "Total: {{count}} protocols",
            searchPlaceholder: "Search protocols...",
            columns: {
                code: "Protocol Code",
                name: "Protocol Name",
                group: "Group",
                executionTime: "Execution Time",
                actions: "Actions",
            },
            create: {
                title: "Create Protocol",
                protocolCode: "Standard Code (SOP)",
                protocolCodePlaceholder: "Enter Protocol Code",
                protocolTitle: "Protocol Name",
                protocolTitlePlaceholder: "Enter Protocol Name",
                protocolSource: "Issuing Organization",
                protocolSourcePlaceholder: "Origin, e.g. TCVN, ISO",
                protocolDescription: "Description / Application",
                protocolDescriptionPlaceholder: "Enter description...",
                documents: "Attached Documents",
                uploadDocument: "Upload document",
                parameters: "Analysis Parameters",
                sampleTypes: "Sample Types",
                chemicals: "Chemicals / Reagents",
                success: "Protocol created successfully",
                error: "Error occurred while saving protocol",
            },
            edit: {
                title: "Edit Protocol",
            },
            detail: {
                generalInfo: "General Info",
                group: "Protocol Group",
                executionTime: "Execution Time",
                description: "Description",
                executionGuide: "Execution Guide",
                equipment: "Required Equipment",
                chemicals: "Chemicals",
            },
            chemicals: {
                name: "Chemical Name",
                cas: "CAS Code",
                amount: "Amount Used",
                bomTitle: "Chemicals Used (BOM)",
                loadFromProtocol: "Load from Protocol",
                searchPlaceholder: "Search chemicals (SKU)...",
                chemicalName: "Chemical Name",
                skuId: "SKU ID",
                consumedQty: "Consumed Qty",
                unit: "Unit",
                emptyBom: "No chemicals added. Search and add above.",
            },
        },
        parameters: {
            parameterId: "Parameter ID",
            parameterName: "Analysis Parameter Name",
            displayStyle: "Display Configuration",
            technicianAlias: "Internal Alias",
            title: "Parameter List",
            total: "Total: {{count}} parameters",
            searchPlaceholder: "Search parameters...",
            allGroups: "All Groups",
            create: {
                title: "Create Parameter",
                parameterName: "Parameter Name",
                parameterNamePlaceholder: "Enter name",
                displayStyle: "Display Style",
                displayStylePlaceholder: "e.g. bold, italic",
                technicianAlias: "Tech Alias",
                technicianAliasPlaceholder: "e.g. CHEM01",
                success: "Parameter created successfully",
            },
            edit: {
                title: "Edit Parameter",
                matrixSection: "Parameter Matrix (Configuration)",
                relatedSection: "Parameters / Samples / Chemicals",
            },
            columns: {
                code: "Code",
                name: "Parameter Name",
                group: "Group",
                matrixCount: "Matrix Count",
            },
            detail: {
                matrices: "Parameter Matrices",
                matrixCode: "Matrix Code",
                feeBeforeTax: "Fee (Pre-tax)",
                feeAfterTax: "Fee (Post-tax)",
                lod: "LOD",
                loq: "LOQ",
            },
        },
        sampleTypes: {
            title: "Sample Type List",
            total: "Total: {{count}} sample types",
            sampleTypeId: "Sample Type ID",
            sampleTypeName: "Sample Group / Product",
            displayTypeStyle: "Multilingual Display Name",
            searchPlaceholder: "Search sample types...",
            table: {
                sampleTypeId: "SampleType ID",
                sampleTypeName: "SampleType Name",
                displayTypeStyle: "Multilingual Display Name",
                createAt: "Created At",
            },
            create: {
                title: "Create New SampleType",
                sampleTypeName: "SampleType Name",
                sampleTypeNamePlaceholder: "Enter SampleType Name",
                displayDefault: "Name Display Default",
                displayDefaultPlaceholder: "Enter Name Display Default",
                displayEng: "English Name",
                displayEngPlaceholder: "Enter English Name",
            },
            createSuccess: "Create SampleType Successfully",
        },
        parameterGroups: {
            title: "Parameter Group List",
            total: "Total: {{count}} parameter groups",
            groupId: "Test Package Code",
            groupName: "Package Name / Parameter Group",
            sampleType: "SampleType",
            matrixIds: "Parameters in Package",
            feeBeforeTaxAndDiscount: "Original Package Price",
            feeBeforeTax: "Package Price (Pre-tax)",
            discountRate: "Package Discount (%)",
            feeAfterTax: "Package Price (Post-tax)",
            searchPlaceholder: "Search parameter groups...",
            create: {
                title: "Create New Parameter Group",
                groupName: "Package Name",
                groupNamePlaceholder: "Enter Package Name",
                matrices: "Matrices",
                matricesPlaceholder: "Select Matrices",
                matricesSearchPlaceholder: "Search Matrices",
                sampleType: "SampleType",
                feeBeforeTaxAndDiscount: "Original Package Price",
                discountRate: "Package Discount (%)",
                feeBeforeTax: "Package Price (Pre-tax)",
                taxRate: "Tax Rate (%)",
                feeAfterTax: "Package Price (Post-tax)",
                groupNote: "Group Note",
                groupNotePlaceholder: "Enter Group Note",
            },
        },
    },
    lab: {
        receipts: {
            receiptId: "Receipt ID",
            receiptCode: "Record Code (Code)",
            receiptStatus: "Record Status",
            receiptDate: "Received Date",
            receiptDeadline: "Deadline (Committed)",
            receiptNote: "Receipt Note",
            receiptPriority: "Priority Level",
            receiptDeliveryMethod: "Delivery Method",
            receiptTrackingNo: "Tracking Number",
            senderInfo: "Sender Info",
            conditionCheck: "Sample Condition",
            reportConfig: "Report Configuration",
            receiptIdPlaceholder: "Enter Receipt ID",
            receiptCodePlaceholder: "Enter Receipt Code",
            status: {
                Pending: "Pending",
                Processing: "Processing",
                Done: "Completed",
                Cancelled: "Cancelled",
            },
            dashboard: {
                totalThisMonth: "Total Receipts This Month",
                overdue: "Overdue Receipts",
                pendingAssignment: "Unassigned Samples",
                waitingReturn: "Waiting for Results",
                tabs: {
                    orders: "Orders",
                    processing: "Processing",
                    return: "Return Results",
                },
                searchPlaceholder: "Search by receipt code, client...",
                createReceipt: "New Receipt",
                daysLeft: "{{days}} days left",
                overdueLabel: "Overdue!",
                noSample: "No samples",
                createBillOfLading: "Create Bill of Lading",
                view: "View",
            },
        },
        samples: {
            sampleId: "Sample ID (Lab ID)",
            receiptId: "Belongs to Receipt",
            sampleTypeId: "Sample Matrix Type",
            productType: "Product Group",
            sampleName: "Sample Name",
            sampleTypeName: "Sample Type Name",
            sampleClientInfo: "Sample Name/Ref (Client)",
            sampleInfo: "Sample Info",
            sampleReceiptInfo: "Receipt Info",
            sampleStatus: "Sample Status",
            sampleVolume: "Sample Volume",
            sampleWeight: "Net Weight",
            samplePreservation: "Preservation Condition",
            sampleStorageLoc: "Storage Location",
            sampleRetentionDate: "Retention Date",
            sampleDisposalDate: "Disposal Date",
            sampleIsReference: "Is Reference Sample",
            samplingInfo: "Field Sampling Info",
            physicalState: "Physical State",
            status: {
                received: "Pending",
                analyzing: "Analyzing",
                stored: "Stored",
                disposed: "Disposed",
            },
            placeholders: {
                receiptId: "Enter receipt ID...",
                sampleTypeId: "Enter sample type ID...",
                sampleClientInfo: "Enter client sample info...",
                sampleVolume: "Enter sample volume...",
                sampleStatus: "Select status...",
                sampleStorageLoc: "Enter storage location...",
            },
            upsert: {
                createTitle: "Create sample",
                updateTitle: "Update sample",
            },
        },
        analyses: {
            price: "Unit Price",
            analysisId: "Analysis ID",
            sampleId: "Sample ID",
            matrixId: "Matrix ID",
            parameterId: "Parameter",
            parameterName: "Parameter Name",
            protocolCode: "Protocol Code",
            technicianId: "Main Tech",
            technicianIds: "Support Techs",
            equipmentId: "Main Equipment",
            analysisStatus: "Analysis Status",
            analysisResult: "Result",
            analysisResultStatus: "Result Evaluation",
            analysisStartedAt: "Started At",
            analysisCompletedAt: "Completed At",
            analysisUncertainty: "Measurement Uncertainty (U)",
            methodmethodLOD: "Applied LOD",
            methodmethodLOQ: "Applied LOQ",
            analysisUnit: "Unit",
            analysisLocation: "Performing Lab",
            qaReview: "Result Approval (QA/QC)",
            status: {
                Pending: "Pending",
                Testing: "Testing",
                Review: "Review",
                Approved: "Approved",
                DataEntered: "Data entered",
                TechReview: "Tech review",
                ReTest: "Re-test",
                Complained: "Complained",
                Cancelled: "Cancelled",
            },
            resultStatus: {
                Pass: "Pass",
                Fail: "Fail",
                NotEvaluated: "Not evaluated",
            },
        },
        equipment: {
            equipmentId: "System ID",
            equipmentName: "Equipment Name",
            equipmentCode: "Asset Code",
            equipmentStatus: "Operating Status",
            equipmentCalibDate: "Last Calibration Date",
            equipmentNextCalib: "Next Calibration Date",
            equipmentLog: "Usage Log",
        },
        inventory_items: {
            itemId: "Item/Chemical Code",
            itemName: "Item/Chemical Name",
            itemType: "Classification",
            itemStockQty: "Stock Quantity",
            itemUnit: "Unit",
            itemLotNo: "Lot No",
            itemExpiryDate: "Expiry Date",
            itemLocation: "Storage Location",
            itemCasNo: "CAS No",
        },
    },
    document: {
        files: {
            fileId: "File ID",
            fileName: "File Name",
            mimeType: "Format",
            fileSize: "Size",
            uris: "Download Link",
            fileTags: "Tags",
        },
        documents: {
            documentId: "Document ID",
            fileId: "Attached File",
            refId: "Reference ID",
            refType: "Reference Type",
            jsonContent: "Source Data",
        },
        reports: {
            reportId: "Report ID",
            receiptId: "Record Code",
            sampleId: "Sample ID",
            header: "Report Header",
            content: "Report Content",
            footer: "Report Footer",
        },
    },
    service: {
        opai: {
            messageOpaiId: "AI Log ID",
            role: "Role",
            content: "Conversation Content",
            tokenUsage: "Token Usage",
            contextId: "Processing Context",
        },
        shipments: {
            shipmentId: "Shipment ID",
            trackingNumber: "Tracking Number",
            provider: "Shipping Provider",
            status: "Delivery Status",
            shipOrder: "Shipment Order Info",
        },
    },
    technician: {
        workspace: {
            title: "Technician Workspace",
            ktv: "Tech",
            totalTasks: "Total Tasks",
            todo: "To Do",
            waiting: "Waiting Approval",
            redo: "Redo",
            searchPlaceholder: "Search by sample code, parameter, method...",
            table: {
                sampleCode: "Sample Code",
                parameterName: "Parameter",
                receivedDate: "Received Date",
                protocol: "Method",
                result: "Result",
                unit: "Unit",
                assignedTo: "Assigned To",
                status: "Status",
                note: "Note",
                actions: "Actions",
                enterResult: "Enter Result",
                enterNote: "Note...",
            },
            status: {
                pending: "To Do",
                inProgress: "In Progress",
                waitingApproval: "Waiting Approval",
                redo: "Redo",
            },
            balance: {
                tabTitle: "Cân phân tích (Data Streamer)",
                port: "Port Connection",
                baudRate: "Baud Rate",
                connect: "Connect Balance",
                disconnect: "Disconnect",
                connected: "Connected to Balance",
                disconnected: "Disconnected",
                connecting: "Connecting...",
                notSupported: "Browser does not support Web Serial API or secure HTTPS environment is required.",
                latestWeight: "Balance Reading",
                stable: "Stable",
                unstable: "Unstable",
                clearData: "Clear Sheets",
                exportCsv: "Export CSV File",
                settings: "COM Port Config",
                skipEmpty: "Skip empty lines (CR/LF)",
                stableOnly: "Only stable readings",
                autoCopy: "Auto-copy clipboard",
                liveStream: "Real-time Data Stream",
                autoFill: "Auto Fill Reading",
                autoNext: "Auto Next Row",
                fillValue: "Get Weight",
                fillValueTooltip: "Feed current weight into input cell (Space / F2)",
            },
        },
    },
    reception: {
        receipts: {
            status: {
                title: "Status",
                draft: "Draft",
                receive: "Receive",
                processing: "Processing",
                completed: "Completed",
                reported: "Reported",
                cancelled: "Cancelled",
            },
            delete: {
                title: "Delete Reception",
                description: "This action can't be undone.",
                confirmLabel: "Are you sure you want to delete this reception?",
                receiptIdLabel: "Receipt Id",
            },
        },
        createReceipt: {
            title: "Create receipt",
            description: "Fill in client and sample information to create a receipt.",
            tabs: {
                basic: "Basic",
                full: "Full",
            },

            clientInfo: "Client info",
            invoiceInfo: "Invoice info",
            contactInfo: "Contact info",
            receiptInfo: "Receipt info",

            samplesList: "Samples",
            noAnalysis: "No analyses",
            analysisList: "Analyses",

            duplicateSample: "Duplicate sample",
            removeSample: "Remove sample",

            addSampleInfo: "Add sample info",
            addAnalysis: "Add analysis",

            cancelButton: "Cancel",
            createButton: "Create",
            createFullButton: "Create (full)",

            receiptPriorityPlaceholder: "Enter priority",
            receiptDeliveryMethodPlaceholder: "Enter delivery",
            trackingNumber: "Tracking Number",
            trackingNumberPlaceholder: "Enter tracking number",
        },
        receiptDetail: {
            title: "Receipt details: {{code}}",
            description: "View receipt information and sample/analysis list.",
            printLabel: "Print label",
            exportHandover: "Export result",

            digitalRecords: "Digital records",
            uploadFile: "Upload file",
            noFile: "No files yet",

            sampleImage: "Sample images",
            noImage: "No images yet",
            sendMail: "Send email",
            sendMailReception: "Reception Email",
            sendMailResult: "Result Email",
            manageImages: "Manage images",
            sendMailTitle: "Send email to client",
            sendMailDesc: "Compose and send an email with receipt information.",

            emailTemplate: {
                from: "",
                subject: "[LIMS] Results for {{code}}",
                content: "Hi {{clientName}},\n\nReceipt: {{receiptCode}}\nReceived date: {{receiptDate}}\nDeadline: {{deadline}}\n\nPlease reply if you need further assistance.\n",
            },

            email: {
                from: "From",
                to: "To",
                cc: "CC",
                bcc: "BCC",
                toPlaceholder: "Enter recipient email addresses (comma separated)...",
                subject: "Subject",
                content: "Content",
                attachments: "Attachments",
                reportAttached: "Attached Report",
                send: "Send Email",
                sending: "Sending...",
                sendSuccess: "Email sent successfully!",
                sendError: "Failed to send email. Please try again.",
            },
        },
        sampleReception: {
            metrics: {
                totalReceipts: "Total receipts",
                overdueReceipts: "Overdue",
                pendingSamples: "Pending samples",
                returnResults: "Return results",
            },

            tabs: {
                incomingRequests: "Incoming Requests",
                processing: "Processing",
                returnResults: "Return results",
            },

            search: {
                placeholder: "Search by receipt code / client / email...",
            },

            actions: {
                createReceipt: "Create receipt",
            },

            receiptInfo: "Receipt info",

            table: {
                processing: {
                    status: "Status",
                    deadline: "Deadline",
                    notes: "Notes",
                    actions: "Actions",
                },
                returnResults: {
                    receiptInfo: "Receipt info",
                    tracking: "Tracking",
                    deadline: "Deadline",
                    contact: "Contact",
                    actions: "Actions",
                },
            },

            deadline: {
                overdue: "Overdue",
                daysLeft: "{{count}} day left",
                daysLeft_plural: "{{count}} days left",
            },

            tracking: {
                none: "No tracking number",
            },

            contact: {
                phoneLabel: "Phone:",
                emailLabel: "Email:",
            },
        },
        incomingRequests: {
            columns: {
                requestId: "Request ID",
                clientName: "Client",
                salePerson: "Sales",
                incomingStatus: "Status",
                orderStatus: "Order",
                paymentStatus: "Payment",
                sampleCount: "Samples",
                analysisCount: "Tests",
            },
            status: {
                New: "New",
                Processing: "Processing",
                Converted: "Converted",
                Rejected: "Rejected",
            },
            emptyState: "No incoming requests yet",
            fastConvert: "Quick Create",
            samplePreview: "Sample & Analysis Details",
            convertSuccess: "Receipt created successfully!",
            convertError: "Failed to create receipt, please try again.",
            convertModal: {
                title: "Quick Create Receipt",
                client: "Client",
                sampleCount: "Samples",
                analysisCount: "Tests",
                willCreate: "Will create:",
                tests: "tests",
                noSamples: "This request has no sample data. Cannot create receipt.",
                submit: "Create Receipt",
            },
        },

        sampleDetail: {
            title: "Sample details: {{code}}",
            description: "View and edit sample info, analyses, and attachments.",
            relatedReceipt: "Related receipt",
            productDetails: "Product details",
            testingInfo: "Testing info",
            analysisList: "Analysis list",
            addAnalysis: "Add analysis",
            attachedFiles: "Attached files",
            priority: "Priority",
            receivedBy: "Received By",
        },
    },

    labManager: {
        dashboard: {
            title: "Lab Management",
            description: "Approve results, manage output and complaints",
            pending: "Pending Approval",
            approved: "Approved",
            delivered: "Delivered",
            complaint: "Complaints",
            searchPlaceholder: "Search by receipt, sample, parameter, tech, client...",
            tabs: {
                pending: "Pending Approval",
                final: "Final Output",
            },
            table: {
                receiptCode: "Receipt Code",
                sampleCode: "Sample Code",
                parameterName: "Parameter",
                protocol: "Method",
                result: "Result",
                assessment: "Assessment",
                technician: "Tech",
                timestamp: "Time",
                note: "Note",
                actions: "Actions",
                customer: "Client",
                customerType: "Client Type",
                sampleCount: "Samples",
                testCount: "Tests",
                completedDate: "Completion Date",
                status: "Status",
                threshold: "Threshold",
                approve: "Approve",
                reject: "Reject",
                view: "View",
                process: "Process",
                notePlaceholder: "Approval note...",
            },
            customerTypes: {
                company: "Company",
                individual: "Individual",
            },
        },
    },
    inventory: {
        nav: {
            chemical: "Chemical Inventory",
            general: "General Inventory",
        },
        dashboard: {
            title: "Inventory & Assets",
            description: "Chemicals, supplies, equipment",
            searchPlaceholder: "Search by name, code, CAS No., location...",
            alerts: {
                chemicalExpiry: "<strong class='font-bold text-destructive'>{{count}}</strong> chemicals expiring in {{days}} days!",
                chemicalWarning: "<strong class='font-bold text-orange-600'>{{count}}</strong> chemicals to watch in {{days}} days.",
                equipmentOverdue: "<strong class='font-bold text-destructive'>{{count}}</strong> equipment overdue for calibration!",
                stockLow: "WARNING: Low Stock!",
                daysLeft: "{{days}} days left",
                overdue: "Overdue {{days}} days",
            },
            tabs: {
                chemicals: "Chemicals",
                glassware: "Glassware",
                equipment: "Equipment",
                supplies: "Supplies",
            },
            table: {
                chemicalName: "Chemical Name",
                casNo: "CAS No.",
                batch: "Batch",
                expiry: "Expiry",
                stock: "Actual Stock",
                glasswareName: "Glassware Name",
                category: "Category",
                totalStock: "Total Qty",
                inUse: "In Use",
                available: "Available",
                broken: "Broken/Damaged",
                equipmentName: "Equipment Name",
                equipmentCode: "Equip Code",
                location: "Location",
                lastCalib: "Last Calibration",
                nextCalib: "Next Calibration",
                status: "Status",
            },
            status: {
                active: "Active",
                maintenance: "Maintenance",
                overdue: "Calibration Overdue",
            },
        },
        chemical: {
            title: "Chemical Management",
            tabs: {
                inventories: "Inventories",
                skus: "Chemical Catalog",
                suppliers: "Suppliers & Catalog",
                transactionBlocks: "Transaction Blocks",
                transactions: "Transaction History",
                auditBlocks: "Audit Blocks",
                auditDetails: "Audit Details",
            },
            inventories: {
                chemicalInventoryId: "Inventory ID",
                chemicalSkuId: "SKU ID",
                chemicalName: "Chemical Name",
                lotNumber: "Lot Number",
                lotShort: "Lot",
                poNumber: "PO/Contract Number",
                manufacturerName: "Manufacturer",
                storageBinLocation: "Storage Location",
                currentAvailableQty: "Available Qty",
                initialQty: "Initial Qty",
                chemicalInventoryStatus: "Status",
                expiryDate: "Expiry Date",
                expDate: "Exp Date",
                mfgDate: "Mfg Date",
                openedAt: "Opened At",
                openedDate: "Opened Date",
                openedExpDays: "Days shelf life after opening",
                openedExpDate: "Exp Date (Open)",
                isOpened: "Is Opened",
                total: "Total: {{count}} items",
                searchPlaceholder: "Search barcode, SKU, lot...",
                selectMode: "Label selection mode:",
                selectModeDesc: 'Click rows to select, then click "Print" to print labels.',
                printLabelTitle: "Print Inventory Labels",
                printLabelDesc: "{{count}} labels will be printed (50×22mm × 2/row)",
                sheetSpec: "Sheet Row: 100×22mm | Label: 50×22mm | Margin: 1.5mm",
                alreadyAdded: "This chemical is already in the list",
                addedByScan: "Added: {{id}}",
                notFound: "Code not found: {{id}}",
                quickScanPlaceholder: "Scan QR code to add...",
                selectFromStock: "Select from Stock",
                status: {
                    Quarantined: "Quarantined",
                    New: "New",
                    InUse: "In Use",
                    Empty: "Empty",
                    Expired: "Expired",
                    Disposed: "Disposed",
                },
                details: {
                    title: "Inventory Details",
                    chemicalInfo: "Chemical Info",
                    timeInfo: "Time Info",
                },
                add: "Add New Item",
                edit: "Edit Item",
                createSuccess: "Item created successfully",
                updateSuccess: "Item updated successfully",
                createFailed: "Creation failed",
                updateFailed: "Update failed",
            },
            skus: {
                chemicalSkuId: "SKU ID",
                chemicalSkuIdPlaceholder: "Leave blank to auto-generate",
                chemicalName: "Chemical Name",
                chemicalCASNumber: "CAS Number",
                chemicalFormula: "Formula",
                chemicalBrand: "Brand",
                catalogNumber: "Catalog Number",
                chemicalGrade: "Grade",
                chemicalBaseUnit: "Base Unit",
                chemicalHazardClass: "Hazard Class",
                chemicalTotalAvailableQty: "Total Available",
                chemicalReorderLevel: "Reorder Level",
                openedExpDays: "Default days after opening",
                hazardInfo: "Hazard Info",
                isControlled: "Controlled",
                minStockLevel: "Min Stock Level",
                total: "Total: {{count}} SKUs",
                searchPlaceholder: "Search SKU, name, CAS...",
                selectPlaceholder: "Select SKU...",
                add: "Add Chemical",
                edit: "Edit SKU",
                mfgShort: "Mfg",
                controlled: "Controlled",
                normal: "Normal",
                hazardNone: "Safe",
                hazardFlammable: "Flammable",
                hazardToxic: "Toxic",
                hazardCorrosive: "Corrosive",
                hazardExplosive: "Explosive",
                details: {
                    title: "Chemical Source (SKU)",
                    baseUnit: "Base Unit",
                    reorderLevel: "Reorder Level",
                    hazardClass: "Hazard Class",
                    isControlled: "Controlled",
                    inventorySnapshot: "Stock Snapshot",
                    supplierInfo: "Supplier Info",
                    addSupplier: "Link Supplier",
                    catalogNo: "Catalog No",
                    brand: "Brand/Manufacturer",
                    price: "Ref Price",
                    leadTime: "Lead Time",
                },
            },
            suppliers: {
                searchPlaceholder: "Search supplier, tax ID...",
                chemicalSupplierId: "Supplier ID",
                supplierName: "Supplier Name",
                supplierTaxCode: "Tax ID",
                contactPhone: "Phone",
                contactEmail: "Email",
                supplierStatus: "Status",
                evalScore: "Evaluation Score",
                addSupplier: "Add Supplier",
                editSupplier: "Edit Supplier",
                supplierProfile: "Supplier Profile",
                legalName: "Legal Name",
                isoCertifications: "ISO Certifications",
                contactInfo: "Contact Info",
                noAddress: "No address updated",
                noContact: "No contact updated",
                suppliedSkus: "Provided SKUs",
                catalogNumber: "Catalog Number",
                createSuccess: "Supplier created successfully",
                updateSuccess: "Supplier updated successfully",
                createFailed: "Creation failed",
                updateFailed: "Update failed",
                supplierAddress: "Address",
                isoPlaceholder: "e.g., ISO 9001:2015",
                contactPersons: "Contact Persons",
                statusLabels: {
                    Active: "Active",
                    Inactive: "Inactive",
                    Blacklisted: "Blacklisted",
                },
            },
            transactionBlocks: {
                chemicalTransactionBlockId: "Block ID",
                chemicalTransactionType: "Type",
                blockStatus: "Status",
                referenceCode: "Ref Code",
                createdAt: "Created At",
                createdById: "Created By",
                total: "Total: {{count}} blocks",
                searchPlaceholder: "Search ID, ref...",
                approve: "Approve",
                printLabel: "Print",
                add: "Create Block",
                details: {
                    title: "Transaction Block Details",
                    approve: "Approve & Lock",
                    type: "Type",
                    status: "Status",
                    creationDate: "Creation Date",
                    reference: "Reference / PO",
                    creator: "Created By",
                    lineItems: "Line Items",
                    itemCode: "Item Code",
                    purpose: "Purpose",
                    test: "Analysis",
                    note: "Note",
                    history: "Process History",
                },
                types: {
                    INBOUND: "Inbound",
                    OUTBOUND: "Outbound",
                    ADJUSTMENT: "Adjustment",
                    TRANSFER: "Transfer",
                },
                statusLabels: {
                    DRAFT: "Draft",
                    PENDING_APPROVAL: "Pending Approval",
                    APPROVED: "Approved",
                    REJECTED: "Rejected",
                },
            },
            transactions: {
                searchPlaceholder: "Search barcode, name...",
                chemicalTransactionId: "TX ID",
                chemicalInventoryId: "Item ID",
                chemicalSkuId: "SKU ID",
                transactionType: "Type",
                transactionQty: "Qty",
                transactionDate: "Date",
                referenceId: "Reference",
                details: {
                    title: "Live Transaction Log",
                    impact: "Impact",
                    change: "Change",
                    note: "Operation Note",
                    extraInfo: "Extra Info",
                },
                reportTitle: "CHEMICAL / MATERIAL TRANSACTION HISTORY REPORT",
                reportEditorTitle: "Export Transaction Report",
                exportReport: "Export Report",
                dateRange: "Date Range",
                fromDate: "From Date",
                toDate: "To Date",
                chemicalSkuOldId: "Old Code",
                unit: "Unit",
                totalWeight: "Gross Weight",
                actionTypeLabels: {
                    INITIAL_ISSUE: "Initial Issue",
                    INBOUND_RECEIVE: "Inbound Receive",
                    OUTBOUND_DISBURSE: "Outbound Disburse",
                    ADJUSTMENT: "Adjustment",
                    DISPOSAL: "Disposal",
                    RECONCILIATION: "Reconciliation",
                    SUPPLEMENTAL: "Supplemental",
                    RETURN: "Return",
                    WASTE: "Waste",
                },
            },
            audit: {
                add: "Create Audit Block",
                scanSuccess: "Matched: {{id}}",
                auditName: "Audit Name",
                searchPlaceholder: "Search audit code...",
                auditBlockId: "Audit ID",
                status: "Status",
                createdAt: "Created At",
                createdById: "Created By",
                completedAt: "Completed At",
                statusLabels: {
                    DRAFT: "Draft",
                    IN_PROGRESS: "In Progress",
                    COMPLETED: "Completed",
                    CANCELLED: "Cancelled",
                },
                details: {
                    title: "Audit Details",
                },
                createSuccess: "Audit block created successfully",
                updateSuccess: "Audit block updated successfully",
                createFailed: "Creation failed",
                updateFailed: "Update failed",
            },
            auditDetails: {
                searchPlaceholder: "Search bottle ID, SKU, audit ID...",
                detailId: "Detail ID",
                auditBlockId: "Audit Block ID",
                skuId: "SKU ID",
                inventoryId: "Bottle ID",
                systemQty: "System Qty",
                actualQty: "Actual Qty",
                variance: "Variance",
                scanned: "Scanned",
            },
            allocateStock: {
                title: "Auto Allocate Chemicals (FEFO)",
                subtitle: "Workflow: Analysis ID ➝ Estimate ➝ Allocate",
                steps: {
                    analysis: "Analysis List",
                    estimate: "Estimation",
                    pickList: "Picking List",
                },
                analysisIdLabel: "Analysis ID",
                analysisIdPlaceholder: "Enter ID (e.g., ALY-001) then press Enter...",
                add: "Add",
                analysisList: "Requirement List ({{count}})",
                continueEstimate: "Continue to Estimate",
                bomTitle: "Chemical Requirement Summary (BOM)",
                table: {
                    skuId: "SKU ID",
                    name: "Chemical Name",
                    totalQty: "Total Required",
                },
                back: "Back",
                createOutbound: "Create Outbound (FEFO)",
                success: "Allocated Successfully!",
                successDesc: "System has automatically selected items based on FEFO rule and generated Picking List below.",
                pickingListTitle: "Picking List (From Warehouse)",
                pickingTable: {
                    inventoryId: "Inventory ID",
                    location: "Location",
                    pickQty: "Pick Qty",
                },
                done: "Finish & Close",
                resetAllocation: "Recalculate",
            },
        },
        general: {
            title: "General Inventory",
            description: "Manage all asset information, equipment, and audit trails of activities.",
            tabs: {
                equipment: "Equipment",
                labTools: "Lab Tools",
                activityLogs: "Audit Trail",
            },
            equipment: {
                title: "Equipment",
                id: "Equip ID",
                name: "Equipment Name",
                code: "Asset/Card ID",
                status: "Status",
                lastCal: "Last Cal",
                nextCal: "Next Cal",
                create: "Add Equipment",
                edit: "Edit Equipment",
                search: "Search equipment...",
            },
            labTools: {
                title: "Lab Tools",
                id: "Tool ID",
                name: "Tool Name",
                code: "Code/Card ID",
                type: "Category",
                status: "Status",
                lastCal: "Last Cal/Test",
                create: "Add Tool",
                edit: "Edit Tool",
                search: "Search tools...",
            },
            assetLogs: {
                title: "Asset Audit Trail",
                logId: "Log ID",
                assetId: "Asset ID (EQP/TOOL)",
                table: "Category",
                logType: "Event",
                description: "Description",
                actionTime: "Time",
                search: "Search logs...",
            },
        },
    },
    hr: {
        dashboard: {
            title: "HR Management",
            description: "Manage accounts, info and staff allocation",
            searchPlaceholder: "Search by name, identity code, email...",
            addIdentity: "Add Identity",
            tabs: {
                all: "All Identities",
                lab: "Lab Staff",
                groups: "Work Groups",
                staff: "Staff",
            },
            table: {
                identity: "Identity",
                code: "Emp Code",
                email: "Email",
                contact: "Contact",
                position: "Position",
                department: "Department",
                role: "Role",
                status: "Status",
                actions: "Actions",
            },
            roles: {
                admin: "Admin",
                manager: "Manager",
                technician: "Technician",
                receptionist: "Receptionist",
            },
            status: {
                active: "Active",
                inactive: "Inactive",
            },
        },
        groups: {
            create: "Create New Group",
            leader: "Leader",
            members: "Members",
            addMember: "Add",
            belongTo: "Belongs to",
        },
        fields: {
            email: "Email",
            alias: "Emp Code",
            name: "Full Name",
            password: "Password",
            status: "Status",
            roles: "Roles",
            identityId: "Identity ID",
            identityGroup: "Department / Group",
            identityPhone: "Phone Number",
            identityNID: "ID Card / NID",
            identityAddress: "Address",
        },
        status: {
            active: "Active",
            inactive: "Inactive",
            blocked: "Blocked",
        },
        roles: {
            ROLE_DIRECTOR: "Lab Director",
            ROLE_TECH_MANAGER: "Technical Manager",
            ROLE_QA_MANAGER: "Quality Manager",
            ROLE_SECTION_HEAD: "Section Head",
            ROLE_VALIDATOR: "QC Officer / Validator",
            ROLE_SENIOR_ANALYST: "Senior Analyst",
            ROLE_TECHNICIAN: "Analyst / Technician",
            ROLE_IPC_INSPECTOR: "In-Process Control (IPC)",
            ROLE_RND_SPECIALIST: "R&D Specialist",
            ROLE_RECEPTIONIST: "Sample Receptionist",
            ROLE_SAMPLER: "Sampler",
            ROLE_SAMPLE_CUSTODIAN: "Sample Custodian",
            ROLE_EQUIPMENT_MGR: "Equipment Manager",
            ROLE_INVENTORY_MGR: "Inventory Manager",
            ROLE_SALES_MANAGER: "Sales Manager",
            ROLE_SALES_EXEC: "Sales Executive",
            ROLE_SALES_COLLABORATOR: "Sales Collaborator",
            ROLE_CS: "Customer Service (CS)",
            ROLE_ACCOUNTANT: "Accountant",
            ROLE_REPORT_OFFICER: "Report Officer",
            ROLE_SUPER_ADMIN: "Super Admin / IT",
            ROLE_DOC_CONTROLLER: "Document Controller",
            ROLE_HSE_OFFICER: "HSE Officer",
            ROLE_ADMIN: "Admin (Legacy)",
            ROLE_CUSTOMER: "Customer",
        },
        permissions: {
            invalidJson: "Invalid permissions JSON",
        },
        create: {
            title: "Add Identity",
            toastSuccess: "Identity created successfully",
        },
        update: {
            title: "Update Identity",
            toastSuccess: "Identity updated successfully",
            cancelPasswordChange: "Cancel password change",
            clickToChangePassword: "Click to change password",
            confirmPasswordChange: "Are you sure you want to change the password for this account?",
            enterNewPassword: "Enter new password...",
        },
        delete: {
            title: "Delete Identity",
            description: "Are you sure you want to delete this employee?",
            toastSuccess: "Identity deleted successfully",
        },
        detail: {
            title: "Identity Details",
            auditTitle: "Audit",
            createdAt: "Created at",
            createdBy: "Created by",
            modifiedAt: "Modified at",
            modifiedBy: "Modified by",
        },
    },
    assignment: {
        title: "Technician Assignment",
        description: "Assign Technicians to perform analyses",
        button: "Assign Technician",
        assignedTechnician: "Assigned Technician",
        assignedGroup: "Assigned Group",
        relatedTechnicians: "Related Technicians",
        modal: {
            title: "Technician Assignment",
            description: "Select a Technician or Group to perform for {{count}} selected analyses.",
            tabs: {
                technician: "Individual",
                group: "Group-based",
            },
            selectPlaceholder: "Select Technician...",
            selectGroupPlaceholder: "Select Group...",
            searchPlaceholder: "Search...",
            groupInCharge: "Lead",
            groupMembers: "Members",
            empty: "No results found",
            confirm: "Confirm",
            cancel: "Cancel",
        },
        success: "Technician assigned successfully",
        error: "Failed to assign. Please try again.",
        notSelected: "Please select a technician",
    },
    handover: {
        title: "Sample Handover",
        description: "Scan tester and sample codes to perform handover",
        scanTester: "Scan Tester Code",
        scanSample: "Scan Sample Code",
        placeholder: {
            tester: "Scan or enter tech ID...",
            sample: "Scan or enter sample ID...",
        },
        info: {
            title: "Handover Info",
            tester: "Tester",
            date: "Handover Time",
            sampleTitle: "Sample Info",
            sampleCode: "Sample Code",
            sampleName: "Sample Name",
            sampleType: "Sample Type",
            receivedCondition: "Condition",
            storageCondition: "Storage",
            analysisList: "Required Analysis List",
        },
        table: {
            stt: "No.",
            parameter: "Parameter",
            protocol: "Method",
            location: "Location",
            unit: "Unit",
            deadline: "Deadline",
        },
        confirm: "Confirm Handover",
        document: {
            description: "Preview and export sample handover record",
            print: "Print",
            exportPDF: "Export PDF",
            close: "Close",
            preview: "The record is made in 02 copies with equal legal validity, each party keeps 01 copy.",
        },
        modal: {
            title: "Handover Samples & Tests",
            receipt: "Receipt",
            totalTests: "Total Tests: {{count}}",
            assigned: "Handed Over: {{count}}",
            unassigned: "Not Handed Over: {{count}}",
            save: "Save Handover",
            selectTester: "Select Tech...",
            assignedTo: "Assigned To:",
            quickAssign: "Quick Assign",
            testsAssigned: "{{count}} tests assigned",
        },
        storedSamples: {
            title: "Stored Samples",
            description: "List of samples being stored and analyzed at the lab",
            stats: {
                total: "Total Stored",
                analyzing: "Analyzing",
                completed: "Completed",
                pending: "Pending",
            },
            searchPlaceholder: "Search by receipt code ....",
            table: {
                code: "Sample Code",
                name: "Name",
                type: "Type",
                category: "Category",
                condition: "Condition",
                quantity: "Quantity",
            },
            detail: {
                description: "Description",
                date: "Handover Date",
                person: "Handed By",
                note: "Note",
                parameterList: "Parameter List:",
            },
            parameters: {
                name: "Parameter",
                group: "Group",
                result: "Result",
                unit: "Unit",
                performer: "Performer",
                status: "Status",
            },
        },
        testerAssignment: {
            title: "Technician Assignment",
            total: "Total Tests",
            unassigned: "Unassigned",
            assigned: "Assigned",
            searchPlaceholder: "Search by sample code, name, parameter...",
            filter: "Filter",
            assign: "Assign",
            modal: {
                title: "Assign Technician",
                description: "Select technician group to assign {{count}} tests",
                group: "Technician Group",
                members: "Members: {{members}}",
            },
        },
    },

    documentCenter: {
        title: "Document Center",
        upload: "Upload Document",
        categories: "Categories",
        totalItems: "Total",
        noData: "No documents found",
        noFiles: "No files found",
        createSuccess: "Document created successfully",
        fileUploaded: "File uploaded successfully",
        tabs: {
            all: "All",
            protocol: "Protocols",
            receipt: "Receipt",
            lab: "Testing",
            report: "Reports",
            sop: "SOP/Procedures",
            other: "Other",
        },
        viewMode: {
            documents: "Documents",
            files: "Files",
        },
        headers: {
            all: "All Documents",
            allDesc: "All documents in the system",
            protocol: "Protocol Documents",
            protocolDesc: "SOP, TCVN, ISO, procedures",
            receipt: "Receipt Documents",
            receiptDesc: "Request forms, contracts, client emails",
            lab: "Testing Documents",
            labDesc: "Test logs, raw data, chromatograms",
            report: "Reports & Results",
            reportDesc: "Hard copy results (scan), drafts",
            sop: "SOP & Procedures",
            sopDesc: "Guidance documents, ISO standards",
            other: "Other Documents",
            otherDesc: "Admin, finance, other",
        },
        col: {
            title: "Title",
            status: "Status",
            ref: "Reference",
            date: "Created",
            actions: "Actions",
            fileName: "File Name",
            type: "Type",
            size: "Size",
        },
        preview: {
            title: "Document Preview",
            uploadedBy: "Uploaded By",
            uploadedDate: "Uploaded Date",
            size: "Size",
            link: "Link",
            preview: "Preview",
            download: "Download",
            select: "Select document to view details",
            openNew: "Open in new tab",
        },
        uploadModal: {
            title: "Create New Document",
            desc: "Enter basic info and attach a file for the document.",
            documentTitle: "Document Title",
            documentTitlePlaceholder: "Enter document title...",
            documentStatus: "Status",
            documentStatusPlaceholder: "Select status...",
            commonKeys: "Common Keys",
            commonKeysPlaceholder: "Enter common keys...",
            tags: "Tags",
            tagsPlaceholder: "Enter tag...",
            file: "Attachment",
            dropzone: "Click or drag & drop file here to upload",
            uploading: "Uploading...",
            saving: "Saving...",
        },
        validation: {
            missingFile: "Please select or upload an attachment",
            missingTitle: "Please enter a document title",
        },
    },
    auth: {
        login: {
            title: "System Login",
            username: "Username",
            password: "Password",
            submit: "Login",
            processing: "Processing...",
            errors: {
                expired: "Session expired. Please log in again.",
                empty: "Please enter username and password.",
                invalid: "Invalid username or password.",
            },
        },
    },
    sampleRequest: {
        institute: {
            name: "INSTITUTE OF NATURAL PRODUCTS RESEARCH AND DEVELOPMENT",
            address: "Add: 12 Phung Khoang 2, Dai Mo, Hanoi",
            tel: "Tel: 024 355 35 355",
            email: "Email: cskh@irdop.org",
        },
        title: "INTERNAL SAMPLE HANDOVER RECORD",
    },
    testReport: {
        institute: {
            organizationName: "Institute for Research and Development of Organic Products",
            departmentName: "Analysis and Testing Dept.",
        },
        title: "Certificate of Analysis",
        ref: "Ref.",
        draft: "DRAFT",
        date: "Date",
        customerInfo: "Customer information",
        sampleInfo: "Sample information",
        clientProvidedInfo: "Information provided by client",
        receiptInformation: "Receipt info",
        sampleLabels: {
            name: "Sample name",
            lot: "LOT no.",
            mfg: "Mfg.",
            placeOfProduction: "Mfr.",
            mfgAddress: "Mfg. address",
            exp: "Exp.",
            declarationNo: "Decl. no.",
            registrationNo: "Reg. no.",
            otherInfo: "Other info",
            receiptDate: "Receipt date",
            storageTime: "Retention period",
            storageCondition: "Retained sample cond.", // Đồng bộ chỉ viết hoa chữ đầu, chống tràn dòng
            testDate: "Test date", // Đã cập nhật theo yêu cầu
            description: "Description",
        },
        table: {
            stt: "No.",
            test: "Parameter",
            result: "Result",
            unit: "Unit",
            protocol: "Method",
            scope: "Accreditation",
        },
        noStorage: "No storage",
        notesTitle: "Notes",
        notes: {
            kph: "Not detected",
            methodLOD: "Limit of detection",
            methodLOQ: "Limit of quantification",
            irdop: "Parameters conducted by IRDOP",
            ex: "Parameters conducted by subcontractors",
            vs: "Accredited per ISO/IEC 17025:2017",
            tdc: "Accredited per Decree 107/2016/ND-CP",
            sampleInfo: "Sample information provided by the customer",
            validOnly: "The results are only valid for the tested sample(s)",
        },
        signatures: {
            qaManager: "Quality Assurance Manager",
            vicePresident: "Vice President"
        }
    },
    equipment: {
        title: "Lab Equipment",
        subtitle: "Manage device data streams, calibration, and status.",
        tabs: {
            balance: "Analytical Balance",
            microscope: "Microscope",
            tlc: "Thin Layer Chromatography (TLC)",
        },
        pending: {
            title: "Feature Under Development",
            description: "The connection and data streaming system for this device is currently being developed. Please check back later.",
        }
    }
};
