...
import { HelpBubble } from "@/components/crm/HelpBubble";
...
export function CRMPage() {
    ...
    return (
        <div className="p-6 space-y-6">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-3">
                    <div className="flex gap-2 bg-muted/50 p-1 rounded-lg">
                        {tabs.map(({ key, Icon, label }) => (
                            <Button
                                ...
                                className={`flex items-center gap-2 ${activeTab === key ? "bg-background shadow-sm text-foreground" : "text-muted-foreground"}`}
                            >
                    ...
                    <div className="flex items-center gap-2 flex-1 w-full md:w-auto md:max-w-md">
                        <div className="relative flex-1">
                            ...
                            <Input placeholder={t("common.search")} value={globalSearch} onChange={(e) => setGlobalSearch(e.target.value)} className="pl-10 bg-background" />
                        </div>

                        <Button type="button" variant="default" className="flex items-center gap-2" onClick={() => setCreateOpen(true)}>
            ...
            <CrmCreateModalHost open={createOpen} tab={activeTab} onClose={() => setCreateOpen(false)} />
            ...
            <HelpBubble guidePath="guide-crm.html" label="HDSD CRM" />
        </div>
    );
}
