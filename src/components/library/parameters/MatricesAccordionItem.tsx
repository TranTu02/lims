...
export function MatricesAccordionItem(props: Props) {
    ...
    const canSave = useMemo(() => {
        ...
        const turnaround = edit.turnaroundTime.trim().length ? toNumberOrNull(edit.turnaroundTime) : 0;
    ...
    const onSave = async () => {
        const feeBeforeTax = toNumberOrNull(edit.feeBeforeTax);
        const feeAfterTax = toNumberOrNull(edit.feeAfterTax);
        const turnaroundTime = edit.turnaroundTime.trim().length ? toNumberOrNull(edit.turnaroundTime) : null;
    ...
    const parameterLabel = matrix.parameterName?.trim() || matrix.parameterId || t("common.noData");
    const sampleTypeLabel = matrix.sampleTypeName?.trim() || matrix.sampleTypeId || "";
    const protocolLabel = matrix.protocolCode?.trim() || matrix.protocolId || "";

    return (
        <div className="border border-border rounded-lg overflow-hidden">
            <button onClick={onToggle} className="w-full px-3 py-2.5 bg-muted/50 hover:bg-muted transition-colors flex items-center justify-between gap-4" type="button">
                <div className="text-left font-medium text-sm text-foreground flex-1 min-w-0">
                    <div className="truncate">{parameterLabel}</div>
                    <div className="text-xs text-muted-foreground mt-0.5 font-normal truncate">
                        {sampleTypeLabel}: {protocolLabel}
                    </div>
                </div>
