import type { OrderDetail, OrdersCreateBody, OrdersUpdateBody, OrderRequestForm, OrderSampleItem, OrderTransaction } from "@/types/crm/order";
...
export function normalizeOrderInitial(initial?: OrderDetail | null): OrderUpsertFormState {
    ...
    return {
        ...
        orderScope: toStr(initial?.orderScope ?? ""),
    };
}
...
function toSamples(values: OrderUpsertFormState): OrderSampleItem[] | null {
    ...
    const mapped: OrderSampleItem[] = values.samples.map((s) => ({
        ...
        analyses: s.analyses.length ? s.analyses.map((a) => ({ matrixId: toNullIfEmpty(a.matrixId) })) : [],
    }));
...
function toRequestFormString(values: OrderUpsertFormState): string | null {
    ...
    const hasAny = Boolean(rf.requestedBy) || Boolean(rf.requestedAt) || Boolean(rf.notes);
...
export function toOrdersCreateBody(values: OrderUpsertFormState): OrdersCreateBody {
    return {
        ...
        contactPerson:
            toNullIfEmpty(values.contactName) || toNullIfEmpty(values.contactEmail) || toNullIfEmpty(values.contactPhone)
                ? {
        ...
        orderScope: toNullIfEmpty(values.orderScope),
    };
}
