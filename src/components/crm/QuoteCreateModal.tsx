...
export function QuoteCreateModal({ open, onClose }: Props) {
    ...
    const mut = useMutation({
        mutationFn: async (body: QuotesCreateBody) => {
            const seed = typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : `${Date.now()}-${Math.floor(Math.random() * 1e9)}`;
            ...
            const created: any = unwrapOrThrow(createdRes as any);
            ...
            return unwrapOrThrow(updateRes as any) as any;
        },
