...
import { useQuoteFull } from "@/api/crm/quotes";
...
export function QuoteUpsertModal({ open, mode, initial, onClose, onSubmit, submitting, onSaved }: Props) {
    const { t } = useTranslation();
    const { data: staffData } = useIdentitiesAll({ query: { itemsPerPage: 1000 } });
    const staff = staffData?.data ?? [];

    const quoteId = toStr(initial?.quoteId).trim();
    const { data: fullData } = useQuoteFull(quoteId, { enabled: open && mode === "update" && !!quoteId });

    const current = fullData || initial;

    const initialForm = useMemo<FormState>(() => {
        const cp = (current?.contactPerson ?? null) as {
            contactName?: unknown;
        ...
        const qs = toStr(current?.quoteStatus).trim();
        const safeStatus: QuoteStatusOption | "" = isQuoteStatus(qs) ? qs : "";

        return {
            quoteId: mode === "update" ? toStr(current?.quoteId) : "",

            quoteCode: toStr(current?.quoteCode),
            clientId: toStr(current?.clientId),
            salePersonId: toStr(current?.salePersonId),
            ...
            taxRate: toStr(current?.taxRate),
            discount: toStr(current?.discount),

            totalFeeBeforeTax: toStr(current?.totalFeeBeforeTax),
            totalFeeBeforeTaxAndDiscount: toStr(current?.totalFeeBeforeTaxAndDiscount),
            totalTaxValue: toStr(current?.totalTaxValue),
            totalDiscountValue: toStr(current?.totalDiscountValue),

            totalAmount: toStr(current?.totalAmount),
            quoteStatus: safeStatus,
            quoteScope: toStr(current?.quoteScope),
        };
    }, [
        current?.clientId,
        ...
        current?.quoteId,
        current?.quoteStatus,
        current?.salePersonId,
        current?.taxRate,
        current?.totalAmount,
        current?.totalDiscountValue,
        current?.totalFeeBeforeTax,
        current?.totalFeeBeforeTaxAndDiscount,
        current?.totalTaxValue,
        current?.quoteScope,
        mode,
    ]);
    ...
    const title = useMemo(() => (mode === "create" ? t("crm.quotes.create.title") : t("crm.quotes.update.title")), [mode, t]);

    const toContactPerson = () => {
        const hasAny = form.contactName || form.contactEmail || form.contactPhone || form.contactPosition;
    ...
    const buildFields = (): QuoteFormFields => ({
        ...
        quoteScope: form.quoteScope || null,
                ...
                >
                    ...
                    <div className="flex items-start justify-between gap-3 px-4 py-3 border-b border-border">
                        <div className="min-w-0">
                            <DialogPrimitive.Title className="text-lg font-semibold text-foreground truncate">{title}</DialogPrimitive.Title>
                        </div>
                                ...
                                <SectionBox title={t("crm.quotes.form.sections.basic")} icon={<ReceiptText className="h-4 w-4" />}>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        ...
                                        <LabeledField label={t("crm.quotes.columns.quoteStatus")}>
                                            <Select value={form.quoteStatus} onValueChange={(v) => setForm((s) => ({ ...s, quoteStatus: (v || "") as QuoteStatusOption | "" }))} disabled={submitting}>
                                                <SelectTrigger>
                                        ...
                                        <LabeledField label={t("crm.quotes.columns.salePersonId")}>
                                            <Select value={form.salePersonId} onValueChange={(v) => setForm((s) => ({ ...s, salePersonId: v }))} disabled={submitting}>
                                                <SelectTrigger>
                                                    <SelectValue placeholder={t("crm.quotes.placeholders.salePersonId")} />
                                                </SelectTrigger>
                                                <SelectContent>
                                                    {staff.map((s) => (
                                        ...
                                        <LabeledField label={t("crm.quotes.columns.scope", { defaultValue: "Phạm vi (Scope)" })}>
                                            <Input
                                                value={form.quoteScope}
                                                onChange={(e) => setForm((s) => ({ ...s, quoteScope: e.target.value }))}
                                                disabled={submitting}
                                                placeholder={t("crm.quotes.placeholders.scope", { defaultValue: "VD: Thực phẩm, Quan trắc..." })}
                                            />
                                        </LabeledField>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        ...
                                        <LabeledField label={t("crm.quotes.columns.totalFeeBeforeTaxAndDiscount")}>
                                            <Input
                                                ...
                                                onChange={(e) => setForm((s) => ({ ...s, totalFeeBeforeTaxAndDiscount: e.target.value }))}
                                                disabled={submitting}
