...
import { useOrdersFull } from "@/api/crm/orders";

import type { OrderDetail, OrderListItem } from "@/types/crm/order";
import { createEmptyOrderFormState, createEmptySample, createEmptyTransaction, normalizeOrderInitial, type OrderUpsertFormState } from "@/components/crm/orderUpsertMapper";
...
function DatePickerField({ label, value, placeholder, onChange, className }: { label: string; value: string; placeholder: string; onChange: (nextIso: string) => void; className?: string }) {
    ...
    return (
        <LabeledField label={label} className={className}>
            <Popover>
                ...
                <PopoverContent align="start" side="bottom" sideOffset={8} collisionPadding={12} className="w-auto min-w-[340px] p-3">
                    <Calendar
                        mode="single"
                        selected={selected}
                        onSelect={(d) => {
...
export function OrderUpsertModal({ open, mode, initial, onClose, onSubmit, submitting }: Props) {
    ...
    const orderId = toStr(getProp(initial, "orderId")).trim();
    const { data: fullData } = useOrdersFull({ params: { orderId } }, { enabled: open && mode === "update" && !!orderId });

    useEffect(() => {
        if (!open) return;
        const base = mode === "update" ? fullData || initial : null;
        if (mode === "update" && base) {
            // eslint-disable-next-line react-hooks/set-state-in-effect
            setForm(normalizeOrderInitial(base as OrderDetail));
            return;
        }
        setForm(createEmptyOrderFormState());
    }, [open, mode, initial, fullData]);

    const title = useMemo(() => (mode === "create" ? t("crm.orders.create.title") : t("crm.orders.update.title")), [mode, t]);
                                ...
                                <SectionBox title={t("crm.orders.form.sections.basic")} icon={<ReceiptText className="h-4 w-4" />}>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        ...
                                        <LabeledField label={t("crm.orders.columns.quoteId")}>
                                            <Input value={form.quoteId} onChange={(e) => setForm((s) => ({ ...s, quoteId: e.target.value }))} placeholder={t("crm.orders.form.placeholders.quoteId")} />
                                        </LabeledField>
                                        ...
                                        <LabeledField label={t("crm.orders.columns.salePersonId")}>
                                            <Select value={form.salePersonId} onValueChange={(v) => setForm((s) => ({ ...s, salePersonId: v }))}>
                                                <SelectTrigger>
                                                    <SelectValue placeholder={t("crm.orders.form.placeholders.salePersonId")} />
                                                </SelectTrigger>
                                                <SelectContent>
                                                    {staff.map((s) => (
                                        ...
                                        <LabeledField label={t("crm.orders.columns.scope", { defaultValue: "Phạm vi (Scope)" })}>
                                            <Input
                                                value={form.orderScope}
                                                onChange={(e) => setForm((s) => ({ ...s, orderScope: e.target.value }))}
                                                placeholder={t("crm.orders.form.placeholders.scope", { defaultValue: "VD: Thực phẩm, Quan trắc..." })}
                                            />
                                        </LabeledField>
                                        ...
                                        <LabeledField label={t("crm.orders.columns.taxRate")}>
                                            <Input value={form.taxRate} onChange={(e) => setForm((s) => ({ ...s, taxRate: e.target.value }))} placeholder={t("crm.orders.form.placeholders.taxRate")} />
                                        </LabeledField>
                                                                        ...
                                                                        setForm((st) => ({
                                                                            ...
                                                                            samples: st.samples.map((x) =>
                                                                                x.id === samp.id
                                                                                    ? {
                                                                                          ...x,
                                                                                          analyses: [...x.analyses, { id: `${Date.now()}_${Math.random()}`, matrixId: "" }],
                                                                                      }
                                                                                    : x,
                                                                            ),
                                                                        }))
                                                                                        ...
                                                                                        setForm((st) => ({
                                                                                            ...
                                                                                            samples: st.samples.map((x) =>
                                                                                                x.id === samp.id
                                                                                                    ? {
                                                                                                          ...x,
                                                                                                          analyses: x.analyses.map((y) => (y.id === a.id ? { ...y, matrixId: e.target.value } : y)),
                                                                                                      }
                                                                                                    : x,
                                                                                            ),
                                                                                        }))
