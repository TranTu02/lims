...
export function InventoryDetailPanel({ inventory, onClose }: Props) {
    ...
    const chemType = displayInv.chemicalType || sku?.chemicalType || "";
    const isPreparedType = ["Hóa chất pha", "Dung dịch chuẩn độ", "Thuốc thử"].includes(chemType);

    const renderContent = () => (
        <>
            <div className="sticky top-0 bg-background border-b border-border px-4 py-3 flex items-start justify-between z-10 pr-14 md:pr-4 shrink-0">
                <div>
                ...
                <div className="flex items-center gap-1">
                    ...
                    <Button variant="ghost" size="sm" onClick={onClose} type="button" className="hidden md:flex">
                        <X className="h-4 w-4" />
                    </Button>
                </div>
            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2 text-xs">
                                <div>
                                    ...
                                    <div className="font-medium">
                                        {String(typeof (displayInv as any).preparedBy === "string" 
                                            ? (displayInv as any).preparedBy 
                                            : (displayInv as any).preparedBy?.identityName || (displayInv as any).preparedById || "-").normalize("NFC")}
    ...
    return (
        <>
            {/* Mobile View: Modal (90% width, 90% height) with a big close button */}
            <div className="fixed inset-0 z-[65] bg-black/60 flex items-center justify-center p-4 md:hidden">
                <div className="w-[90vw] h-[90vh] bg-background rounded-xl border border-border flex flex-col overflow-hidden relative">
                    <button
                        type="button"
                        onClick={onClose}
                        className="absolute top-3 right-3 z-[70] p-2.5 bg-muted hover:bg-muted/80 text-muted-foreground hover:text-foreground rounded-full shadow-md border border-border flex items-center justify-center transition-colors"
            ...
            <div className="hidden md:flex flex-col w-96 lg:w-[450px] shrink-0 bg-background rounded-lg border border-border overflow-hidden max-h-[calc(100vh-140px)] sticky top-[72px]">
                {renderContent()}
            </div>
