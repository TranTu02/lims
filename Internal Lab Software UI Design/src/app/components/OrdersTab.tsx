              ...
              <tr key={order.orderId} className="hover:bg-gray-50">
                <td className="px-4 py-4">
                  <div className="space-y-1">
                    ...
                    {order.salePerson && (
                      <div className="text-xs text-gray-500">Sale: {order.salePerson}</div>
                    )}
                  </div>
                </td>
                <td className="px-4 py-4">
                ...
                <td className="px-4 py-4 text-right">
                  <div className="space-y-1">
                    ...
                    <div className="text-xs text-gray-500">
                      Trước thuế: {formatCurrency(order.totalFeeBeforeTax)}
                    </div>
                  </div>
                </td>
                <td className="px-4 py-4 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    {order.paymentStatus === 'paid' && (
                      <Badge variant="default" className="bg-blue-500 text-xs">Đã thanh toán</Badge>
                    )}
                  </div>
                </td>
                <td className="px-4 py-4 text-center">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {order.samples.reduce((sum, s) => sum + s.analyses.length, 0)} chỉ tiêu
                  </div>
                </td>
                <td className="px-4 py-4 text-center">
                ...
                <td className="px-4 py-4 text-center">
                  {order.receiptId && onReceiptClick ? (
                    <button
                      onClick={() => onReceiptClick(order.receiptId!)}
                  ...
                  <div className="flex items-center justify-center gap-2">
                    {order.receiptId ? (
                      <Badge
                        ...
                        onClick={() => onReceiptClick && onReceiptClick(order.receiptId)}
                      >
                        Đã tiếp nhận
                      </Badge>
                    ) : (
